...
// accent fill (0/50/100% width) on a panel2 base and carries two invisible
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex" role="group" aria-label="Rating">
        {[1, 2, 3, 4, 5].map((star) => (
          <span key={star} className="relative inline-block h-[26px] w-[26px] select-none">
            <span
              aria-hidden
              className="absolute inset-0 text-center text-[22px] leading-[26px] text-panel2"
            >
              ★
            </span>
            <span
              aria-hidden
              className="absolute inset-y-0 left-0 overflow-hidden text-[22px] leading-[26px] text-acc"
              style={{ width: current >= star ? "100%" : current >= star - 0.5 ? "50%" : "0%" }}
            >
              <span className="block w-[26px] text-center">★</span>
            </span>
            <button
              type="button"
              disabled={disabled}
      ...
      <span className="min-w-[60px] text-[12.5px] font-medium text-mut">
